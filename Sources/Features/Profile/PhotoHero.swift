...
final class PhotoHeroCoordinator: ObservableObject {
    ...
    /// `onChangeAvatar` — не nil только для СВОЕЙ аватарки: добавляет пункт «Изменить
    /// фото профиля» в меню «...» и вызывается ПОСЛЕ закрытия галереи (окно уже снято,
    /// иначе следующий sheet/picker откроется под ним — оно на уровень выше основного).
    func present(photos: [Photo], index: Int, post: Post?, from sourceView: UIView,
                 onChangeAvatar: (() -> Void)? = nil) {
        ...
        let gallery = PhotoGalleryController(
            ...
            sourceView: sourceView, deps: self, onChangeAvatar: onChangeAvatar
        )
...
final class PhotoGalleryController: UIViewController, UIScrollViewDelegate {
    ...
    var onClosed: (() -> Void)?
    /// Не nil только для просмотра СВОЕЙ аватарки — добавляет пункт в меню «...».
    private let onChangeAvatar: (() -> Void)?
    /// Взведён тапом на «Изменить фото профиля»: после закрытия (см. finishClose)
    /// зовём onChangeAvatar — окно галереи к этому моменту уже снято координатором.
    private var changeAvatarRequested = false
    ...
    init(photos: [Photo], index: Int, post: Post?,
         sourceView: UIView, deps: PhotoHeroCoordinator, onChangeAvatar: (() -> Void)? = nil) {
        self.photos = photos
        self.index = index
    ...
    /// Хвост закрытия: сначала обычная очистка/снятие окна (onClosed, коордиатор),
    /// ЗАТЕМ — если попросили — открытие смены аватарки (уже на основном окне).
    private func finishClose() {
        onClosed?()
        if changeAvatarRequested { onChangeAvatar?() }
    }
    ...
    @objc private func commentsTapped() {
        ...
        let root = CommentsView(ownerID: post.ownerID, postID: post.postID, fallbackIDs: [], post: post)
    ...
    @objc private func menuTapped() {
        ...
        if onChangeAvatar != nil {
            sheet.addAction(UIAlertAction(title: "Изменить фото профиля", style: .default) { [weak self] _ in
                self?.changeAvatarRequested = true
                self?.close()
            })
        }
...
enum PhotoActions {
    // @MainActor: note() рисует тост (UILabel) — обязан идти с главного потока
    // (URLSession/PHPhotoLibrary свою фоновую работу делают сами при await).
